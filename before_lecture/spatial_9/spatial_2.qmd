...
when you don't have a better way to filter the data
--maps (rectangular b/c screens are rectangular)
--very fast, because compares just X and Y values instead of doing a spatial join
...
```{python}
data.buffer(500).plot(edgecolor="white") #500 meters
plt.axis("off")
plt.show()
```
...
```{python}
data["dense"] = 0
# Filter rows with above average pop density and updat
data.loc[data["pop_density_500"] > data["pop_density_k
data.dense.value_counts()]]

dissolved = data[["pop2019", "area_km2", "dense", "geo
    by="dense", aggfunc="sum"]]

#aggregation step set index to "dense", reset to defau
dissolved = dissolved.reset_index()
dissolved
